...
|---|---|---|---|
| TypeScript / JavaScript | Full AST (native, swc) | `.ts, .tsx, .js, .jsx, .mjs, .cjs, .mts, .cts` | Symbols, imports/dep graph, calls, HTTP provides/consumes across Express/Hono/NestJS/Next.js/tRPC and more, router-mount fragments, middleware guard attributes, ORM `db-table` facts (Prisma client accessors and TypeORM `@Entity` classes / `@InjectRepository`/`getRepository` references), and ORM-LESS `db-table` consumes — the tables named inside a raw SQL statement string (`env.DB.prepare("SELECT … FROM ledger")`, `` sql`…` ``), read through `parser-sql` so the key matches the migration-side provide. Recognition needs the string to OPEN a statement head with UPPERCASE keywords: that is the only thing separating a query from English prose like "Select a date from the list", which is a structurally valid `SELECT`. Lower-case SQL is therefore not recognized (a silent under-report, never a claim of "no table access"), and an interpolated table name (`` `… FROM ${t}` ``) is dropped rather than guessed. Also RUNTIME ASSET-URL references — the string paths handed to the browser loaders that fetch a file by URL instead of importing it (`new Worker`/`new SharedWorker`, `importScripts`, `new URL(p, import.meta.url)`, `AudioWorklet.addModule`, `navigator.serviceWorker.register`). These are invisible to the static import graph, so a `public/`-served worklet loaded only this way reads as `fan_in == 0`; the path is captured verbatim and the engine resolves it against the tree's `public/`/`static/` root, bumping the target's fan-in and seeding it as an `unreachable` entrypoint, so it is neither a `dead-candidates` false positive nor a false island. NO `dep` node or edge is added, which is what keeps the bump from minting the very finding it exists to prevent. `fetch`/`<img src>`/`<link href>` are deliberately not captured (non-eligible extensions or server routes — no dead-candidates benefit, real reachability-FP surface), a scheme-carrying string (`https:`, `data:`, `blob:`) is not a file reference and is skipped, and `serviceWorker.register` alone is rebased to served-absolute at capture — on the stated ASSUMPTION that the registering page is served at the site root, so an app served under a sub-path resolves to the wrong target |
...
| C# | Full CST (native, tree-sitter-c-sharp 0.23.5) | `.cs` | Symbols (top-level + nested class/interface/struct/enum/record/delegate as dot-qualified `Outer.Inner` names, methods/constructors/properties with body spans, `const`/`static readonly` fields; `public`-modifier exports), imports/dep graph (`using` directives incl. `static`/alias/`global`, resolved by a namespace→files index — a `using Foo.Bar;` fans out to every file declaring namespace `Foo.Bar`, the same package-directory-fanout honesty Go/Java use), ASP.NET Core HTTP route provides (`[ApiController]`/`[Controller]` attribute controllers with class `[Route("api/[controller]")]` + method `[HttpGet]`/`[HttpPost("{id}")]`/… composition and the `[controller]` token, plus Minimal-API `app.MapGet` literal routes with their `MapGroup` prefix composed through builder chains and cross-statement group variables), `HttpClient` literal HTTP egress consumes (`GetAsync`/`PostAsync`/`GetFromJsonAsync`/… with `$"…"` interpolation reassembly), EF Core ORM `db-table` provides (`DbSet<T>` properties named by EF's property-name convention, `[Table("…")]` attribute renames, and fluent `ToTable("…")` inside an `IEntityTypeConfiguration<T>` class — both of the latter override the convention, and the fluent one is the shape .NET actually writes: measured 2026-09-11, `[Table(` reaches 0 files across eShop/be-aspnet/aspnetcore while `IEntityTypeConfiguration<` reaches 9 in eShop alone. The gate is the EF `using` OR a STRUCTURAL signal — a class in the file deriving from `DbContext` — because a project that keeps its EF usings in a `GlobalUsings.cs` declares none in the context file itself, and requiring the import there extracted nothing from an ordinary layout. The attribute gate widens WITH it rather than separately, and that pairing is the substance: reading `DbSet<T>` under the structural signal while still requiring the import for `[Table]` does not under-report, it MIS-reports — it emits the convention name for an entity that renamed itself, so a `[Table("app_users")]` entity came out as `table:users` and joined a migration it has nothing to do with; a non-literal `[Table]` argument skips the class rather than guessing), and CALL SITES for the projected call-site CHANNEL (`console-write` + `env-read` families, read by `call-scan` DSL rules — the same NOT-the-call-graph caveat the Go row states: a C# route never enters the handler-reachability BFS and is exempted by extension rather than false-positived) |
...
all it needs). So is anything living only inside an inline `mod foo { ... }` block: the symbol-keyed
layers — symbols, imports, call attribution, axum router recognition — do not walk inline-mod bodies,
so a guard call, an import, or a route that exists only there is invisible (silence about it is scope,
not a verdict), while the line-keyed facts (call sites, test spans) still see the whole file.
See `zzop_parser_rust`'s own crate doc for the exact v1 gaps.
...
`DefaultServeMux`/`NewServeMux` (including Go 1.22's `"METHOD /path"` pattern syntax), routes registered
on a project's OWN router type (see the paragraph below), and `net/http`'s
...
recognized — roadmap, not attempted.

A THIRD provide-side lane has no import to gate on at all, because the router it reads is the
project's own type — the dominant real-world Go layout, and one both gates above are blind to by
construction. It keys on the shape of the registration call instead: `<identifier>.<Title-case
verb>("<rooted literal>", …)`, plus the callback sub-router form (`r.Group("/api", func(api R) { … })`)
whose prefix is composed by lexical descent. `Handle`/`HandleFunc` stay `net/http`'s, and a file
importing gin is left to gin's reader, so no route is read twice. Two limits are load-bearing and
deliberate: a catch-all registration emits ONE unknown-method route rather than one per verb, and a
receiver whose mount point is decided outside the file — a router taken as a function parameter, or a
prefix that is a runtime configuration value — emits NOTHING, because a route published at a path the
server does not answer is worse than a route left absent. What that shape can NOT tell you is how many
routes it did not read; `zzop coverage`'s zero-extraction census reports a language that yielded
nothing at all, and stays silent on partial loss by construction.

`tree-sitter-go` is a full CST (not merely lexical), but this
...
convention-named provide simply never joins; plus fluent `ToTable("…")` inside an
`IEntityTypeConfiguration<T>` class, whose literal is the PHYSICAL table name and whose entity comes
from the class's base-list type argument) are extracted natively today. Minimal-API route-group
prefixes compose across a chain (`MapGroup` under any number of other builder calls) AND across
statements (`var g = app.MapGroup("/api"); g.MapGet(...)`), from a file-scoped declarator map; a name
that map cannot resolve — unknown, or declared twice with conflicting prefixes — is SKIPPED rather than
keyed prefix-less. Roadmap: the fluent LAMBDA overload `modelBuilder.Entity<T>(b => b.ToTable("…"))`
and the chained `modelBuilder.Entity<T>().ToTable(...)` spelling, CROSS-FILE suppression of the
`DbSet` convention name a fluent `ToTable` overrides (per-file io projection cannot express it, so both
keys emit today), the schema segment (`HasDefaultSchema`) that no language's io key carries, query-site
`db-table` consumes, cross-file base-controller `[Route]` inheritance,
`HttpClient.SendAsync(HttpRequestMessage)`, conventional routing (`MapControllerRoute`), and
SDK-injected implicit/`global` usings beyond what the source itself declares. Namespace
...
**An entry that no longer holds what zzop wrote is refused, not served.** Every entry carries a hash
binding its payload to its key, checked on every read. A payload that does not match — a hand-edit, a
partial overwrite, disk corruption — is recomputed from source instead of served, and the run says how
many entries that happened to rather than healing silently. The limit is worth stating plainly: that
hash is keyless and public, so it detects an edit that did not know about it, not one that recomputed
it. It is a corruption and tampering **detector**, not an authenticity check. The directory is local
and gitignored, so this was never a repository-borne vector; the case it closes is a well-formed entry
whose findings were emptied by hand, which used to delete those findings from every later run with
nothing in the output to contradict it.
...
The split described here holds **from v0.30.0 onward**. Up to and including v0.29.1 the schema version
carried the release number, so *every* upgrade wiped the cache whether or not anything about the analysis
had changed, and there was no size cap at all. That single value was doing both jobs above, which meant
the only way to reclaim disk was to charge every user a full cold run on every release.
...
The join itself carries five integrity gates on top of the raw `(kind, key)` match:
- **Wildcard partition**: a route whose path is an ANT **pattern** rather than a key
  (`@GetMapping("/files/**")`) is lifted OUT of the join entirely instead of being compared literally.
  Read as a literal, one such route produced three wrong answers from one cause: the live catch-all was
  reported as a dead route (`unconsumedProvides`), and every call beneath it as a missing one
  (`unprovidedConsumes`). A prefix/longest-match fallback was rejected — that would make edges out of a
  guess and would have to invent a tiebreak between two catch-alls, and this join's contract is that it
  is exact. So the route emits no edge, is never reported dead, and the calls it serves (same verb, ANT
  path match — a `POST` beneath a GET-only catch-all still reports) leave `unprovidedConsumes`. **Edge
  count is unchanged by construction**; the payoff is false findings removed, not links added. Because
  that correctness is bought with silence, every partitioned route is disclosed with the number of call
  sites it swallowed — on the declaring tree's own `warnings` (so it reaches `zzop cross`'s
  `sources[].warnings`) and in `crossLayer.wildcardRoutePartitions`. The predicate
  (`zzop_core::wildcard_route_covers`) is shared with the single-tree `unprovided-consume` rule, like
  route identity below.
...
## Non-goals — where the boundaries are written

zzop's boundaries are documented next to the mechanism each one bounds, not restated here — this
section only collects the doors so an evaluator does not have to discover them one paragraph at a
time. Each link is the owning statement; if a sentence here and a linked one ever disagree, the
linked one is the truth.

- **Per-language framework scope** — what each language's native extraction does NOT recognize
  (Flask; Rocket/warp/actix-web; echo/chi/fiber; JAX-RS/Micronaut/Quarkus — and the envelope door
  each gap leaves open): [Language support](#language-support).
- **Symbol-layer gaps in Rust** — macro-argument identifiers and inline `mod` bodies:
  [Language support](#language-support), and `zzop_parser_rust`'s crate doc for the exact list.
- **Deployment-supplied routing** — gateway prefixes, environment-variable bases, and
  deployment-config rewrites stay unresolved unless a config declares them:
  [Cross-layer join](#cross-layer-join) and [Sentinel-based tree rewrites](#sentinel-based-tree-rewrites).
- **What a finding's severity does and does not promise** — confidence is bounded by what the run
  could see: [getting-started — reading a reply](getting-started.md).
- **What is frozen at 1.0 and what is explicitly not** — the compatibility surface, and the list of
  things (exact finding sets included) that never joined it:
  [VERSIONING — the compatibility surface](../VERSIONING.md#the-compatibility-surface) and
  [explicitly NOT part of it](../VERSIONING.md#explicitly-not-part-of-the-compatibility-surface).
- **No runtime, no dataflow proof, no code generation** — zzop reads source lexically and
  structurally, never executes it, and writes no code; lexical rules match textual order, not proven
  dataflow. Per-rule limits live in each rule's own row: [rules/catalog.md](rules/catalog.md).
