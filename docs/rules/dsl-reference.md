...
| `next_line_exclude_pattern` | regex \| null | `null` | One-line-**lookahead** veto, the mirror of `prev_line_exclude_pattern` and under the same contract: a matched line is skipped when the **immediately following** line matches this regex. For the case where the trigger sits on the line that *opens* an options object and the option itself lands one line down (`express.json({` / `  limit: ...`), which no same-line veto can reach. Exactly one line, never a window — a span knob would make how-far-to-look a per-rule policy value, and an unbounded walk is how a veto starts silencing genuine findings elsewhere in the same function. Same `strip_string_literals` masking as every other line regex; the **last** line of a file has no successor and can never be vetoed here, the mirror of line 0. |
| `enclosing_call_exclude_pattern` | regex \| null | `null` | **Upward** veto over the *enclosing expression*, and the only **multi-line** field here: a matched line is skipped when the joined text of the still-unclosed `(`/`[`/`{` **opener lines** above it matches this regex. Compiled with a `(?m)` prefix, so `^`/`$` anchor per opener line. The window is *not* a span of source — a line enters it only by carrying a bracket still open at the site, and a group the walk watches close (`])`, `})`) consumes its opener — which is why a 40-line reach does not go body-scoped the way an unbounded lookback would. Exists for the case where following the rule's own prescription would **break** the code: `await prisma.$transaction([ a.create(...), b.create(...) ])` takes un-awaited query builders, so `db/unawaited-write`'s "await the call" moves each element out of the transaction. Require the enclosing call's *own* promise to be consumed (`\b(?:await\|return\|yield)\b[^\n]*...`) — an unawaited enclosing call leaves the inner writes genuinely unawaited. Every ambiguity declines and the site keeps firing (a multi-line template literal, a comment that is not whole-line-and-bracket-free, a mismatched bracket, an opener past the cap); a rule leaning on it must disclose those residuals in its message. |
...
| `trigger_call_exclude_pattern` | regex \| null | `null` | The **narrow** veto beside `absent`'s wide one: a trigger match does not count when this regex matches inside the **trigger call's own parentheses** — that match's line plus the continuation lines its unclosed parens hold open, capped at 8 (`veto_window::MAX_CALL_WINDOW_LINES`), matched in multi-line mode. Same "not a hit at all" semantics as `trigger_in_loop`/`after`: a vetoed match neither satisfies the trigger nor anchors the finding, so a **second, unmitigated** trigger call elsewhere in the span still fires. Exists because `absent` reads the whole symbol *body*, which is the wrong scope for "this call's own argument was checked" — a body-scoped veto also waives a call whose neighbouring statement merely mentions a guard-shaped name. The window rather than the anchor line alone is load-bearing: the mitigator a formatter pushed onto the **next** line, inside the still-open call, is the common shape (measured on cal.com: of the 17 `security/open-redirect` sites the veto clears, 16 carry the helper on a CONTINUATION line and one on the anchor line). Residuals, every one of which leaves the site **firing** because this field only ever suppresses: the 8-line cap; a comment anywhere inside the argument list, which ends the window at its own line; a mitigator applied on a *preceding* statement or decided by a wrapper, out of reach by construction; and a line carrying **two** trigger matches, which declines the window outright rather than guessing which call it belongs to. **The field supplies the TEXT; what a match PROVES is the pattern's business** — a pattern that stops at a guard call's opening `(` has proved only that the guard OPENS the argument, so `sink(safe(x) + req.query.q)` is waived by it. A rule claiming the guard produces the guard's WHOLE value has to spell the value's closure as well; `browser.json`'s `${html-sink-sanitized}` fragment is the repo's older spelling of that (balanced argument list, then `[;,)}]|$`), and `security/open-redirect` is the method-scan one. |
...
| `line_exclude_pattern` | regex \| null | `null` | Lexical **veto** on the site's own source line — the negated mirror of `line_pattern`, which the `regex` crate's lack of lookaround makes unspellable as a positive pattern. Its one use is a **declaration the source makes about the call** (`security/weak-crypto` reads CPython's `usedforsecurity=False`, the argument bandit honours as B324). Degrades the **opposite** way from `line_pattern`, deliberately: it only ever suppresses, so a site whose line the file text cannot supply (envelope mode carries no source lines) keeps **firing** — no line, no declaration witnessed, no waiver. It reaches the call's **own** line only, so a construction split across lines puts the declaration out of reach; a rule setting it must say so in its message rather than imply whole-call coverage. |
