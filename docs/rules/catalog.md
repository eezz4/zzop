...
|---|---|---|---|---|
| `update-delete-no-where` | critical | method-scan | `no-where-ok` | `updateMany`/`deleteMany` called with no populated `where:` clause anywhere in the enclosing function — a whole-table write. An empty `where: {}` counts as no filter (Prisma writes every row) and still fires; a populated, multi-line, or dynamic `where:` vetoes. |
| `pagination-no-orderby` | warning | method-scan | `pagination-ok` | `skip`/`take` pagination used with no populated `orderBy` anywhere in the enclosing function — page boundaries can shift between requests without a stable sort. An empty `orderBy: {}` (no ordering applied) counts as none and still fires. |
...
`rules/native/rules-cross-layer` owns the 24 `cross-layer/*` ids (multi-tree cross-layer join rules);
...
toggle/gating surface). `zzop_engine::register_all_native` composes all five. The 24
...
`crossLayer` in `analyzeTrees`'s output. None of the 24 honor an inline suppression marker (disable-only, via
