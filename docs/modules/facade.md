...
| Field | Type | Meaning |
|---|---|---|
| `extensions` | `{ext, files, structural, kind}[]`, extension-ascending | One row per extension contributing **zero** resolved import edges while being a principal filetype. Two legs: at least 10% of the tree's FILES **and** at least 10% of its LINES. (A largest-file exclusion was added to the line leg on 2026-08-20 and reverted the same day — it erased a row whenever one member dominated its extension, which is the MyBatis-mapper shape this cell exists for. How a filetype spreads its lines is not evidence about whether anything read it; a `package-lock.json`-shaped row is disclosed by its `kind` instead.) Or — for an extension that parsed — at least 10% of the files with a structural projection plus a non-zero `coverage.declaredImportsByExt` entry, a leg that carries no line test at all. The first leg — the NO-PARSER kind, `structural: 0` — carries a third condition the second does not: the filetype must be one where a dispatch-`None` CAN have cost something (the engine's own `extraction_can_lose_facts` test, borrowed through `zzop_facade`), so prose, stylesheets, images, fonts, media and archives are excluded outright while SOURCE languages and DATA/CONFIG formats both qualify. That test asks about a STRUCTURAL projection, not about every fact a file can hold: since 2026-08-20 a `.md` page's `<script setup>` block DOES contribute `import` lines to the dep graph (VitePress/Nuxt Content compile each page as a Vue SFC), and since 2026-08-21 a `.mdx` page's BARE top-level `import` statements do the same (measured: 175 of the dogfood corpus's 218 `.mdx` files carry one — `find corpus ( -name .git -o -name node_modules -o -name .zzop ) -prune -o -type f -iname '*.mdx' -print0 | xargs -0 grep -l '^import ' | wc -l`), as in-edges only in both cases — their symbols and io stay unprojected, which is why prose remains on the excluded side of this gate rather than moving and are told apart by the row's `kind` (`"source"` | `"data-config"`). The parsed leg does not ask that question, because a parser did claim those files and there is no dispatch-`None` to price. The gate was `is_non_source_extension` for one review cycle, which answers the DIFFERENT question the "bring a parser adapter" warning asks, and it suppressed the largest extraction gap in the dogfood corpus — macrozheng/mall's 114 `.xml` MyBatis mappers (15.8% of files, 11.5% of lines, 906 SQL statements) produced an empty list on this surface AND on `unreadExtensions`. A `data-config` row is a place to LOOK, not a verdict: an i18n `locales/*.json` directory and a mapper directory are the same row until the reader opens them, and this build does not read them to find out. `structural: 0` covers TWO causes with opposite remedies — no parser claimed the extension, or one claimed every file and bailed; `coverage.degraded` tells them apart (directus: `{"ext": "vue", "files": 587, "structural": 0}` beside 306 `unimported-export` and 126 `dead-candidates` findings); `structural > 0` is the parsed-but-unresolved kind (gogs: `{"ext": "js", "files": 173, "structural": 172}` beside 173 `dead-candidates`, one per file). |
...
| `meaning` | `string` | The vocabulary, riding inside the object it describes (same device as `ruleTimings`/`architecture.painMeaning`). |
...
**Two shares, not one, and the line share is asked of the POPULATION rather than of the total.** Every
leg is a measurement rather than a filetype vocabulary, and each exists to stop a skew the others
cannot see. Requiring both shares at once stops the two SCATTERED skews: an extension spread over many
small files buys the file leg and fails the line leg, one spread over a few large files does the
reverse, and an asset directory is usually one or the other. The mirror image had no defence until
2026-08-20: a handful of build-tool manifests passes the file leg, and ONE generated artifact can buy
the line leg outright. Hence the subtraction — a share a single file produces single-handedly is a
statement about that file, not about a filetype the tree is made of. `is_principal_population` in
`crates/summary/src/analyze/coverage_gaps/census.rs` owns that rule, the corpus measurements on both
sides of it, and the cost it accepts: a tree whose entire population of an extension IS one enormous
file loses its row here. That file is not hidden — it is walked, line-counted and on `zzop coverage`'s
per-extension table; what it is not is a filetype the tree is made of, which is the only claim this
cell makes. The percentages live with the predicate rather than in a second copy here, because a copy
of a measurement is the thing this page keeps having to fix.

**`queryCoverage` asks a narrower version of the same question.** Its own capability cells cover the
PARSER half and gate on FILE share alone — no line leg, and therefore no subtraction — so the two lists
can differ at the margin; the field's `meaning` says so on the wire. What they do NOT differ on is
which filetypes are eligible — the two are comparable on exactly that axis, since `unreadExtensions`
and this field's no-parser leg both describe filetypes nothing parsed: both read
`zzop_engine::extraction_can_lose_facts` through `zzop-facade`'s re-export (that crate is below this
reply's shaping layer, which ships nothing under `zzop-facade`), so that question keeps one owner.
