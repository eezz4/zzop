...
mod orchestrator_tests {
    ...
    // --- prismaSchemaAnalysisWithUsage (producer -> core.analyze_schema_with_usage) ---
    ...
    fn with_usage_parses_and_returns_issues_plus_model_risk() {
        ...
        let result = prisma_schema_analysis_with_usage(
            dir.path(),
            "be",
            DEFAULT_STORE_FACTORY_FN,
            DEFAULT_PRISMA_CLIENT_GETTER_FN,
        );
        let result = result.expect("expected analysis");
        assert_eq!(
            result
        ...
        assert!(result
            .issues
            .iter()
            .any(|i| i.rule == "fk-no-index" && i.field.as_deref() == Some("customerId")));
        assert!(result
            .issues
            .iter()
            .any(|i| i.rule == "implicit-fk" && i.field.as_deref() == Some("customerId")));
        let rules: Vec<&str> = result.issues.iter().map(|i| i.rule.as_str()).collect();
        assert!(rules.contains(&"missing-timestamps"));
        assert!(result
            .issues
            .iter()
            .any(|i| i.rule == "float-money" && i.field.as_deref() == Some("totalAmount")));
        assert!(result
            .issues
            .iter()
            .any(|i| i.rule == "stale-updated-at" && i.field.as_deref() == Some("updatedAt")));
        // cross-check (no store binding) -> dead-model fires.
        assert!(rules.contains(&"dead-model"));
        assert!(result.model_risk["Invoice"] > 0);
    }
    ...
    fn with_usage_finds_schema_via_layout_agnostic_prisma_dir_directly_under_app_dir() {
        ...
        // usage-based rules fire (no bound store)
        assert!(result.issues.iter().any(|i| i.rule == "dead-model"));
        // structure-only result also includes float-money
        assert!(result
            .issues
            .iter()
            .any(|i| i.rule == "float-money" && i.field.as_deref() == Some("amount")));
    }
