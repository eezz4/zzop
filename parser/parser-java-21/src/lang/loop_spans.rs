...
//! mirrors `zzop_parser_go::lang::loop_spans`'s cross-language contract exactly (see
//! `zzop_core::dsl::SourceFile::loop_spans`'s doc for the shared definition every parser projects
//! onto): a call sitting textually inside one of these spans is proven to run once per iteration,
//! not just "somewhere in the same function".
//!
//! ## What is a span here — the eager/lazy boundary
//! Java's four STATEMENT loop forms, each its own grammar kind ([`LOOP_STATEMENT_KINDS`]): classic
//! `for_statement`, `enhanced_for_statement` (`for (T x : xs)`), `while_statement`, and
//! `do_statement`. Every one is the node's OWN whole span, header line included — a call in the
//! loop's condition/update clause runs once per iteration too, the same header-inclusive convention
//! TS/Go pin. A `do`-statement's trailing `while (cond);` line is part of its node span, and its
//! condition likewise re-evaluates per iteration.
//!
