...
/// on but that never reached `PINNED_NODE_KINDS`. The forward test above only asks whether the listed
...
/// merely looks like one contributes nothing.
#[test]
fn every_grammar_node_kind_literal_in_this_crate_is_pinned() {
    ...
    assert!(
        unpinned.is_empty(),
        "these tree-sitter-go NAMED kinds are matched somewhere in src/ but missing from \
