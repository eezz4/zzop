...
| `parser-prisma` | parses the Prisma schema DSL — lexical |
...
Rust (`parser-rust`, syn 2) and Go (`parser-go`, tree-sitter-go) have since cleared the same bar — the
table above is the current native list. A native crate's v1 scope can still be deliberately narrower
