...
//! (`zzop-mcp analyze|analyze-envelope|cross|endpoint`) that share the same handlers. This module is pure protocol
...
/// CLI `zzop-mcp analyze <path>` — default filters. Thin re-export: all shaping lives in
...
/// CLI `zzop-mcp analyze-envelope <envelope.json>` — default filters. Thin re-export: all shaping
...
/// CLI `zzop-mcp cross <path>...` / `zzop-mcp cross --config <path>` — default filters. Thin
...
/// `check_endpoint` MCP tool / `zzop-mcp endpoint` CLI subcommand — thin re-export: the tree
