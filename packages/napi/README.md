...
loader: the actual analysis runs in the `zzop-napi` Rust crate, exposed as a `.node` addon.
...
`packages/napi/zzop-napi.node`. There is currently no automated musl or WASM build path — contributions
...
packages (`@zzop/native` plus the 5 `npm/<platform>/` sub-packages).
