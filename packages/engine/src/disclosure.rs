...
pub const BLINDNESS_REGISTRY: &[BlindnessClass] = &[
    ...
    BlindnessClass {
        ...
        group: EXTRACTION_BLIND,
        summary: "A file in a language with no adapter is read as a degraded lexical fallback and counted \
                  in `coverage.degraded`, but is not broken out per language, so \"this backend does not \
                  serve X\" can be an unparsed-language gap.",
