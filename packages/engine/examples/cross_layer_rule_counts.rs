...
//!
//! Usage: `cargo run --release -p zpz-engine --example cross_layer_rule_counts -- <root> [<root> ...]`
...
fn main() {
    ...
    let mut by_rule: BTreeMap<&str, Vec<&zpz_core::Finding>> = BTreeMap::new();
    ...
    // ZPZ_DUMP_MESSAGES=<n>: print the first n full finding messages per rule (useful for checking
    // message wording, not just counts). Off by default to keep runs terse.
    let dump: usize = std::env::var("ZPZ_DUMP_MESSAGES")
        .ok()
