  ...
  try {
    ...
    throw new Error(
      `zzop-napi: failed to load the native addon for "${platformKey}".\n` +
        ...
        'then copy the produced binary (.dll/.dylib/.so) to packages/native/zzop-napi.node. ' +
        'See packages/native/README.md for details.'
    );
  }
}
...
//
// `packsDir` uses MERGE semantics, not replace: the Rust side (`crates/facade/src/lib.rs`'s
