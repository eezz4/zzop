...
// (packages/native/package.json) instead of six. Plain Node, no dependencies.
//
...
const napiDir = path.join(__dirname, '..');
...
function main() {
  const rootPkgPath = path.join(napiDir, 'package.json');
  ...
  for (const dir of platformDirs) {
    ...
    if (pkg.version !== version) {
      ...
      changed += 1;
      console.log(`sync-versions: ${path.relative(napiDir, pkgPath)} -> ${version}`);
    }
