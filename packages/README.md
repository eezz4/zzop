...
| `check_endpoint` | Definitive answer to "is io key X provided/consumed/joined?" — case-insensitive substring match over every io key (http routes, DB tables, tRPC procedures), one sealed verdict (`linked` / `provided-only` / `consumed-unprovided` / `external` / `unresolved-only` / `ambiguous` / `mixed` / `not-found`) instead of bucket counts to eyeball. |
| `check_coverage` | Definitive answer to "how much of this stack does zzop SEE?" — the reply to read before trusting a zero from the tools above. Per-extension dispatch counts (MEASURED), the compiled-in recognizer and native-analysis rosters including what this build ships OFF by default (CAPABILITY), the per-rule blind-spot cross and the principal filetypes no structural parser read (CAPABILITY×MEASURED), and `unmeasured` — recall, never measured on your tree — as a FIELD rather than a caveat. By ruling there is NO single coverage score: folding the axes into one number either manufactures a recall claim or drops the axis that matters. Takes 1+ paths (unlike `cross_repo`) and lists no findings. |
| `module_map` | Definitive answer to "what IS this codebase?" — the import graph collapsed into MODULES and the imports between them, as data. The orientation reply, and small on purpose: measured over the wire 2026-09-15, `fold` 1 on zzop's own 1741-file tree is 3,032 bytes (10 modules, 6 module edges) where `analyze_repo` on the same tree is 30,587 and carries no module map at all. A module is the first `fold` segments of a path; `fold` is required and is the only knob (the picture lane's own word for the same collapse: `zzop graph --domain dep --fold <n>`), because NOTHING is capped — the answer to "too large" is a coarser grain, never a dropped row. A TOPOLOGY, never a verdict: no severity, no score, no ranking, no findings. Takes 1+ paths, like `check_coverage`. CLI twin `zzop map`. |
