...
//! `zpz_engine::analyze::collect_git` as the default `zpz_git::CollectOptions::commit_type_patterns`
//! override. Lives here rather than in `zpz-git`: that crate owns collection plus the injectable
//! classification *mechanism* only, not the domain vocabulary for what a "FIX" commit looks like.
//! `zpz-git` keeps its own small test-only copy of this table so it need not depend on this crate —
//! see `zpz_git::tags`'s test module doc for why the content is intentionally duplicated there.
...
/// `zpz_git::tags::extract_tags`'s doc for the full bracket-vs-keyword grammar this table feeds.
...
mod tests {
    ...
    //! REVERT-before-FIX ordering, pattern override) by `zpz_git::tags`'s tests against its own copy
