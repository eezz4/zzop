// Bundled starter copy — generated by `zzop init adapter`.
//
// SOURCE OF TRUTH: examples/adapter-kit/lib/envelope.js (the canonical implementation) and
// docs/adapters/envelope.schema.json (the JSON Schema this builder's output satisfies). This file is
// copied from the adapter-kit at `zzop` CLI authoring time because the kit itself is NOT published to
// npm (see docs/adapters/README.md) — this scaffold has to be self-contained. If the envelope contract
// changes upstream (docs/adapters/README.md's "Envelope schema & versioning policy"), re-copy this
...
export class EnvelopeBuilder {
   ...
   * normalized (see lib/keys.mjs's `normalizeProvideKey`). */
  addProvide(filePath, { kind, key, line, symbol } = {}) {
    ...
    if (typeof key !== 'string' || key.length === 0) {
      throw new Error('addProvide: key is required and must already be normalized (see lib/keys.mjs)');
    }
