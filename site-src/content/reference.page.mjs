...
export default {
  ...
  slots: {
    ...
    p16: {
      ...
      en: `<code>analyze_trees_json</code> output + <code>{ pattern: string }</code> &rarr; definitive endpoint/io-key answer. Pure post-processing over an already-produced multi-tree analysis — no re-analysis: <code>pattern</code> is case-insensitively substring-matched against every cross-layer io key (HTTP routes, DB tables, tRPC procedures, plus <code>raw</code> for unresolved consumes), and the result carries a sealed <code>verdict</code> vocabulary (<code>linked</code> | <code>provided-only</code> | <code>consumed-unprovided</code> | <code>external</code> | <code>unresolved-only</code> | <code>ambiguous</code> | <code>mixed</code> | <code>not-found</code>) with the matches, counts, and related findings behind it. Errs on a malformed query and on single-tree <code>analyze_json</code> output — a guided error, since verdicts are join facts (run <code>analyze_trees_json</code>, which joins even a single tree). Backs <code>zzop endpoint</code> / the <code>check_endpoint</code> MCP tool.`,
    },
    ...
    // Inserted between `adapterOverlays` (95/96) and `parsers` (97) after the numbering around them
    // was already published, so it takes a NAMED key rather than renumbering every slot below it —
    // the same escape hatch `summaryCriticalTruncated` and `summaryNativeAnalyses*` already use.
    summaryScores: {
      ko: `<div class="cmd-row"><span class="cmd-row__name"><code>scores</code><span class="cmd-row__type">object &mdash; 기본값 <code>{}</code></span></span><span class="cmd-row__desc">구조 점수가 셀 수 있는 파일의 범위 — 점수만 움직이고 발견은 움직이지 않는다.<span class="cmd-row__more">계약</span></span></div>`,
      en: `<div class="cmd-row"><span class="cmd-row__name"><code>scores</code><span class="cmd-row__type">object &mdash; default <code>{}</code></span></span><span class="cmd-row__desc">Which files the structural scores are allowed to count — scores only, never a finding.<span class="cmd-row__more">contract</span></span></div>`,
    },

    pScores1: {
      ko: `모양: <code>{ excludeTestFilesFromFileMetrics?: boolean }</code>, 기본값 <code>false</code>. 구조 점수는 전부 "이 트리의 파일들" 위에서 잡은 비율이거나 평균이고, 그 모집단에는 기본적으로 당신의 테스트가 들어 있다 — 테스트가 소스보다 많은 레포는 출하되지 않는 코드에 대해 대체로 계산된 답을 받는다. 이 키를 켜면 공유 테스트 경로 관습(<code>zzop_core::is_test_file</code>)이 맞히는 경로가 전부 모집단에서 빠지고, 당신이 선언한 <code>vocabulary.extraTestPathPatterns</code> 가 그 범위를 넓힌다. 객체를 생략한 것은 <code>false</code> 와 같은 뜻이고, 그것이 지금까지 모든 점수가 써 온 모집단이므로 기존 호출자의 수치는 움직이지 않는다.`,
      en: `Shape: <code>{ excludeTestFilesFromFileMetrics?: boolean }</code>, default <code>false</code>. Every structural score is a ratio or an average over &ldquo;the files in this tree&rdquo;, and by default that population includes your tests — so a repo with more test code than source gets answers computed largely about code that never ships. Setting it removes every path the shared test-path conventions match (<code>zzop_core::is_test_file</code>) from that population, widened by your own <code>vocabulary.extraTestPathPatterns</code>. An absent object means <code>false</code>, which is the population every score has always used, so an existing caller's numbers do not move.`,
    },

    pScores2: {
      ko: `<strong>점수만이다.</strong> 어떤 룰도 이 키를 읽지 않으므로 발견이 생기지도 사라지지도 않는다: <code>express</code> 코퍼스 트리에서 재 보니 <code>findings.total</code> 은 56 으로 그대로였고 <code>bySeverity</code>/<code>byRule</code> 은 바이트까지 같았으며, 그 사이 <code>architecture.pain</code> 은 30.0 에서 11.5 로 움직였다. 제외된 파일도 여전히 파싱되고, 간선을 모두 지닌 노드로 남고, 여전히 보고 대상이다 — 잃는 것은 점수의 <em>주어</em> 자격뿐이다.`,
      en: `<strong>Scores only.</strong> No rule reads it, so no finding appears or disappears: measured on the <code>express</code> corpus tree, <code>findings.total</code> stayed 56 and <code>bySeverity</code>/<code>byRule</code> came back byte-identical while <code>architecture.pain</code> moved 30.0 &rarr; 11.5. An excluded file is still parsed, still a node carrying all its edges, and still reportable; what it loses is standing as the <em>subject</em> of a score.`,
    },

    pScores3: {
      ko: `그리고 <em>일부</em> 점수에만 닿는다: 파일에 키가 잡힌 열한 개 지표와 그 위의 <code>pain</code> 롤업까지다. 디렉터리 롤업에 키가 잡힌 넷(<code>cohesion</code>, <code>sdp</code>, <code>mainSequence</code>, <code>modularity</code>)에는 닿지 않고, 점수가 아닌 <code>criticalTop</code>/<code>topRecommendation</code> 에도 닿지 않는다. 그래서 테스트 파일을 뺀 <code>pain</code> 수치 옆에 테스트 모듈이 <code>criticalTop</code> 으로 이름이 실릴 수 있다 — 같은 <code>express</code> 실행에서 키를 끄고 켠 두 판의 <code>criticalTop</code> 세 칸은 바이트까지 같았고, 그 둘째 칸이 <code>test/support/utils.js</code> 였다. 이 키가 <code>pain</code> 의 기준을 다시 잡으므로, 응답은 그 실행의 모집단을 <code>painMeaning</code> 안에, <code>POPULATION:</code> 토큰으로 시작하는 절로 적는다. 두 <code>pain</code> 값은 그 문장이 양쪽에서 똑같이 읽힐 때에만 비교 가능하다.`,
      en: `And only of <em>some</em> scores: it reaches the eleven metrics keyed on a FILE, plus the <code>pain</code> rollup over them. It does not reach the four keyed on a directory rollup (<code>cohesion</code>, <code>sdp</code>, <code>mainSequence</code>, <code>modularity</code>), and it does not reach <code>criticalTop</code>/<code>topRecommendation</code>, which are not scores. So a test module can still be named in <code>criticalTop</code> beside a <code>pain</code> number that excluded test files — on that same <code>express</code> run the three <code>criticalTop</code> slots were byte-identical with the key off and on, the second of them <code>test/support/utils.js</code>. Because the key re-bases <code>pain</code>, every reply states the run's population inside <code>painMeaning</code>, in a clause led by a <code>POPULATION:</code> token; two <code>pain</code> values are comparable only when that sentence reads the same in both.`,
    },
    ...
    summaryCriticalTruncated: {
      ko: `<div class="cmd-row"><span class="cmd-row__name"><code>criticalTruncated</code><span class="cmd-row__type">number</span></span><span class="cmd-row__desc">위 목록의 상한(20)이 <strong>몇 행을 버렸나</strong>. <code>0</code> 이면 전량이고, <code>0</code> 도 항상 실린다.<span class="cmd-row__more">계약</span></span></div>`,
      en: `<div class="cmd-row"><span class="cmd-row__name"><code>criticalTruncated</code><span class="cmd-row__type">number</span></span><span class="cmd-row__desc">How many rows the list above’s cap (20) <strong>dropped</strong>. <code>0</code> means complete, and <code>0</code> is always serialized.<span class="cmd-row__more">contract</span></span></div>`,
    },

    pCriticalTruncated: {
      ko: `상한이 <strong>몇 행을 버렸는지</strong>는 그 상한과 같은 자리에서만 알 수 있고, 리플라이 어디에서도 되짚을 수 없다 — <code>architecture.criticalTop</code> 의 범례는 자기가 들어 올린 <em>세 개</em>만 말하므로, 배열까지 파고든 독자는 아무 문장도 이름 대지 않는 20행 벽을 만난다. <code>목록 길이 + 이 수</code> 가 전체를 복원한다.`,
      en: `How many rows a cap dropped is knowable only at the cap, and nowhere else in the reply — <code>architecture.criticalTop</code>’s legend publishes only the <em>three</em> paths it lifts, so a reader who drilled into the array met a 20-row wall no sentence named. <code>list length + this number</code> reconstructs the full set.`,
    },
    p127: {
    ...
    pNativeAnalyses1: {
      ko: `모양: <code>{ registered, disabled, shippedOff, reportedInCrossLayerFindings }</code>. <code>registered</code> 는 이 빌드가 담은 내장 분석의 수 — 아래 세 목록을 읽을 때의 <strong>분모</strong>다. 세는 것은 <strong>게이트 id 공간</strong>(<code>rules</code>/<code>disabledRules</code> 가 부르는 이름)이지 발견이 실을 수 있는 id 가 아니다: 일부는 발견을 안 내는 점수 계산을 게이트하고, 일부는 발견이 더 잘게 <code>schema/&lt;label&gt;</code> 로 나가는 우산 id 다.`,
      en: `Shape: <code>{ registered, disabled, shippedOff, reportedInCrossLayerFindings }</code>. <code>registered</code> is how many native analyses this build carries — the denominator the three lists are read against. It counts the <strong>gate id space</strong> (what <code>rules</code>/<code>disabledRules</code> name), not the ids a finding can carry: some gate score computations that emit no finding, and some are umbrella ids whose findings carry finer <code>schema/&lt;label&gt;</code> names.`,
    },

    pNativeAnalyses2: {
      ko: `목록을 가르는 이유는 처방이 정반대라서다. <code>disabled</code> = 당신의 config 가 껐다 — <code>findings</code> 에 없다는 것은 <strong>분석 안 됨</strong>이고, 판정을 받으려면 다시 켜라. <code>shippedOff</code> = <strong>이 빌드가</strong> 껐고 당신의 config 가 안 켰다. 분석 안 된 것은 같지만 <em>누가 골랐나</em>가 다르고, 그래서 처방도 다르다 — 끄기를 그만두는 것이 아니라 <code>rules</code> 에 심각도를 적어 이름을 부른다(<code>"dead-candidates": "info"</code>). 미사용 코드 위생 셋(<code>unimported-export</code> · <code>dead-candidates</code> · <code>unreachable</code>)이 그것이고, 도그푸드 코퍼스에서 전체 발견의 61.7% 였다. <code>reportedInCrossLayerFindings</code> = <strong>켜져 있는데도</strong> 여기 나올 수 없다. 이들은 크로스트리 조인을 판정하고 그 자신의 <code>crossLayerFindings</code> 채널로 보고하는데, 트리별 출력에는 그 채널이 없다 — 같은 config 로 크로스레이어 조인을 돌리면 보인다. 등록됐는데 <strong>세 목록 어디에도</strong> 없으면 그것은 돌았다 — 위 문단이 이름 댄 부류만 예외다. 점수 계산을 게이트하는 id 는 발견을 아예 안 내고, 우산 id 의 발견은 더 잘게 <code>schema/&lt;label&gt;</code> 이름으로 나온다. 어느 쪽도 <code>registered</code> 가 세는 자기 이름으로는 <code>findings</code> 에 키를 만들지 않으니, 거기가 비어 있는 것은 그 id 들의 평소 모습이지 판정이 아니다(우산이면 <code>schema/&lt;label&gt;</code> 쪽을 찾아라). 그 밖에 세 목록 어디에도 없는 것은 <code>findings</code> 에 없는 것이 실측된 0 이다.`,
      en: `The lists are kept apart because the remedies are opposite. <code>disabled</code> = your config switched these off, so their absence from <code>findings</code> means <strong>not analyzed</strong>; turn one back on to get a verdict. <code>shippedOff</code> = <strong>this build</strong> switched these off and your config did not turn them on. Same non-evaluation, different author — and so a different remedy: not "stop disabling it" but name it in <code>rules</code> with a severity (<code>"dead-candidates": "info"</code>). What ships off is unused-code hygiene (<code>unimported-export</code>, <code>dead-candidates</code>, <code>unreachable</code>), 61.7% of every finding across the dogfood corpus. <code>reportedInCrossLayerFindings</code> = these are switched <strong>on</strong> and still cannot appear here, because they judge the cross-tree join and report into its own <code>crossLayerFindings</code> channel, which a per-tree output does not have; run the cross-layer join over the same config to see them. Anything registered and in <strong>none of the three</strong> lists ran — except for the kinds the paragraph above named. A score-gating id emits no finding at all, and an umbrella id's findings arrive under the finer <code>schema/&lt;label&gt;</code> names, so neither can key <code>findings</code> under the name <code>registered</code> counted: a blank there is how those ids always look, not a verdict (for an umbrella, look under <code>schema/&lt;label&gt;</code> instead). For everything else in none of the three, absence from <code>findings</code> is a measured zero.`,
    },

    pNativeAnalyses3: {
      ko: `목록은 하나하나 비어 있어도 항상 실린다(<code>[]</code> 포함) — <code>zeroAdmissionRules</code> 와 달리, 고의로. 항목이 있을 때만 나오는 목록은 깨끗한 실행에서 아무 말도 안 하고, 그 바이트는 <em>보고하지 않는 빌드</em>와 구분되지 않는다. 이 객체가 없애려는 혼동이 바로 그것이다.`,
      en: `Every list is always serialized, <code>[]</code> included — unlike <code>zeroAdmissionRules</code>, deliberately. A list that appears only when it has entries reports nothing on a clean run in bytes indistinguishable from a build that does not report at all, which is the confusion this object exists to end.`,
    },
