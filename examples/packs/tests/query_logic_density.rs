//! Exercises `examples/packs/sql-preferences.json`'s `query-logic-density` line-scan rule, moved
//! verbatim from `rules/dsl/sql/query_logic_density.rs` when the rule left the bundle. Only the pack
//! path and the qualified id changed.
...
fn typescript_switch_cases_do_not_fire_query_logic_density() {
    ...
    assert!(
        !out.findings
            .iter()
            .any(|f| f.rule_id == "sql-preferences/query-logic-density"),
        "{:?}",
        out.findings
    );
}
