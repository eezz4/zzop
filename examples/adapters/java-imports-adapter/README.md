...
extracts imports natively, and this adapter's recall is a strict subset of it, so on today's Java
trees the overlay contributes nothing (see Contract points). That is a statement about THIS example's
recall, not about overlays generally: since 2026-07-30 an overlay's dep-graph facts merge additively,
so an adapter that resolves an import the native parser could not still lands its edge. The example
stays as the reference for the recipe itself, which applies unchanged to any extension whose native
support is missing a channel.
