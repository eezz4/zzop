...
fn keys_call_with_glob_string_literal_is_flagged() {
    ...
    let out = scan(&dir);
    let h = hits(&out, "keys-command-in-code");
...
fn scan_cursor_iteration_is_not_flagged() {
    ...
    let out = scan(&dir);
    assert!(
        hits(&out, "keys-command-in-code").is_empty(),
        "{:?}",
        out.findings
    );
}
...
fn redis_keys_ok_marker_above_the_call_suppresses_the_finding() {
    ...
    dir.write(
        "src/debug.ts",
        "declare const client: any;\nexport async function debugListAllKeys() {\n  // zzop-keys-command-in-code-ok: offline debug script, tiny fixed keyspace\n  return client.keys(\"*\");\n}\n",
    );
    let out = scan(&dir);
    assert!(
        hits(&out, "keys-command-in-code").is_empty(),
