...
fn get_endpoint_with_no_cache_marker_is_flagged() {
    ...
    let out = scan(&dir);
    assert_eq!(
        hits(&out, "get-route-no-cache-marker").len(),
        1,
        "{:?}",
        out.findings
    );
...
fn cache_marker_on_the_same_line_suppresses_the_finding() {
    ...
    let out = scan(&dir);
    assert!(
        hits(&out, "get-route-no-cache-marker").is_empty(),
        "{:?}",
        out.findings
    );
}
...
fn read_model_ok_marker_on_the_same_line_suppresses_the_finding() {
    ...
    dir.write(
        "src/routes/apiRoutes.ts",
        "import { Hono } from \"hono\";\nconst apiRoutes = new Hono();\ndeclare const api: any;\napiRoutes.get(\"/api/items\", api.itemList); // zzop-get-route-no-cache-marker-ok: legacy endpoint, cache handled at CDN layer\nexport { apiRoutes };\n",
    );
    let out = scan(&dir);
    assert!(
        hits(&out, "get-route-no-cache-marker").is_empty(),
...
fn read_model_ok_marker_one_line_before_suppresses_the_finding() {
    ...
    dir.write(
        "src/routes/apiRoutes.ts",
        "import { Hono } from \"hono\";\nconst apiRoutes = new Hono();\ndeclare const api: any;\n// zzop-get-route-no-cache-marker-ok: static data served at edge, no server cache needed\napiRoutes.get(\"/api/items/:id\", api.itemDetail);\nexport { apiRoutes };\n",
    );
    let out = scan(&dir);
    assert!(
        hits(&out, "get-route-no-cache-marker").is_empty(),
...
fn read_model_ok_marker_two_lines_before_does_not_suppress() {
    ...
    dir.write(
        "src/routes/apiRoutes.ts",
        "import { Hono } from \"hono\";\nconst apiRoutes = new Hono();\ndeclare const api: any;\n// zzop-get-route-no-cache-marker-ok: static data, no cache needed\n// line 2\napiRoutes.get(\"/api/feed\", api.feed);\nexport { apiRoutes };\n",
    );
    let out = scan(&dir);
    assert_eq!(
        hits(&out, "get-route-no-cache-marker").len(),
...
fn read_model_ok_marker_four_lines_before_does_not_suppress() {
    ...
    dir.write(
        "src/routes/apiRoutes.ts",
        "import { Hono } from \"hono\";\nconst apiRoutes = new Hono();\ndeclare const api: any;\n// zzop-get-route-no-cache-marker-ok: this is too far above\n// line 2\n// line 3\n// line 4\napiRoutes.get(\"/api/feed\", api.feed);\nexport { apiRoutes };\n",
    );
    let out = scan(&dir);
    assert_eq!(
        hits(&out, "get-route-no-cache-marker").len(),
