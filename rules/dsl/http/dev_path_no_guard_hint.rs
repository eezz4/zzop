...
fn dev_path_with_no_env_guard_is_flagged() {
    ...
    let out = scan(&dir);
    assert_eq!(
        hits(&out, "dev-path-no-guard-hint").len(),
        1,
        "{:?}",
        out.findings
    );
...
fn the_emitted_message_states_the_carve_outs_real_shape_not_a_wider_one() {
    ...
    let out = scan(&dir);
    let finding = hits(&out, "dev-path-no-guard-hint")
        .into_iter()
        .next()
...
fn route_exposure_ok_marker_on_the_same_line_suppresses_the_finding() {
    ...
    dir.write(
        "src/routes/apiRoutes.ts",
        "declare const apiRoutes: any;\ndeclare const api: any;\napiRoutes.get(\"/api/dev/config\", api.configHandler); // zzop-dev-path-no-guard-hint-ok: reviewed, disabled outside CI\n",
    );
    let out = scan(&dir);
    assert!(
        hits(&out, "dev-path-no-guard-hint").is_empty(),
