//! End-to-end tests for `rules/dsl/db/db.json`, exercised via `zzop_engine::analyze_tree` so
...
/// Loads the real `db.json` pack, filtered so this test is unaffected by sibling packs under
...
fn db_pack() -> RulePackDef {
    static PACK: std::sync::OnceLock<RulePackDef> = std::sync::OnceLock::new();
    PACK.get_or_init(db_pack_uncached).clone()
}

fn db_pack_uncached() -> RulePackDef {
    ...
    result
        ...
        .map(|(_, pack)| pack)
        .find(|p| p.id == "db")
        .expect("db pack present")
}

fn config() -> EngineConfig {
    EngineConfig {
        source_id: "db-fixture".to_string(),
        packs: vec![db_pack()],
...
fn hits<'a>(out: &'a AnalyzeOutput, rule: &str) -> Vec<&'a zzop_core::Finding> {
    out.findings
        .iter()
        .filter(|f| f.rule_id == format!("db/{rule}"))
        .collect()
}
