//! `api-in-loop` tests, exercised via `zzop_engine::analyze_tree` against real swc-parsed TypeScript
//! fixtures. A `// zzop-api-in-loop-ok` marker on the finding's own line, or the line directly above,
//! suppresses it via `RuleDef::suppress_marker`. Most fixtures use a top-level function rather than a
//! class method to avoid double-counting from overlapping class/method spans; see
//! `overlapping_class_and_method_spans_do_not_double_count` below for that case.
//!
//! Arrived from `rules/dsl/perf/` on 2026-09-03 (see the pack root's module doc for why). Its fixtures
//! are single-file and address one rule, so it keeps the one-shot `scan(rel, content)` it was written
//! against instead of the pack root's `scan(&TempDir)` + `hits(&out, rule)` pair — the two helpers below
//! are private to this module and deliberately SHADOW the glob-imported pair from `super`.
...
fn scan(rel: &str, content: &str) -> Vec<Finding> {
    let dir = TempDir::new("zzop-reliability-apiloop");
    dir.write(rel, content);
    super::scan(&dir)
        .findings
        .into_iter()
        .filter(|f| f.rule_id == "reliability/api-in-loop")
        .collect()
}
