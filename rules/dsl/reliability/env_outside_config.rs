...
fn process_env_nonnull_assertion_outside_config_fires_both_env_rules_on_the_same_line() {
    // Documented interplay (reliability.json's env-outside-config message): env-nonnull-assert (deferred-crash risk of `!`) and env-outside-config (scattered env access) are different concerns, so both firing on the same line is intended, not a duplicate.
    let dir = TempDir::new("zzop-be-rel");
    dir.write(
        "src/handler.ts",
