...
//!
//! Arrived from `rules/dsl/react/` on 2026-09-03 (see the pack root's module doc for why). The rule
//! flags a `setX(...)` state setter co-occurring with an `await` in the same function, gated on the file
//! looking like React (`useEffect`/`useState`/`from 'react'`) and vetoed by any unmount/abort guard token
//! (`AbortController`/`AbortSignal`/`isMounted`/`mountedRef`/`signal:`/`cancelled`/`didCancel`) anywhere in
//! that function's body. That is a CO-OCCURRENCE heuristic — not proof of order, and not proof of unmount
//! risk. The fixtures below pin the accepted false-positive and false-negative shapes.
