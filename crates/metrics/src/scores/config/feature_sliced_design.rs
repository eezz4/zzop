...
//! ([`FeatureSlicedDesignConfig`], declared through `vocabulary.feature_sliced_design.*`) and the precompiled matcher built from it
//! ([`FeatureSlicedDesignMatcher`]).
//!
...
pub struct FeatureSlicedDesignConfig {
...
/// `vocabulary.feature_sliced_design.*` replaces, and the ONE place each list lives (`zzop_engine`'s
...
impl Default for FeatureSlicedDesignConfig {
    fn default() -> Self {
        FeatureSlicedDesignConfig {
...
#[derive(Debug, Clone)]
pub struct FeatureSlicedDesignMatcher {
    pub config: FeatureSlicedDesignConfig,
    pub entry_re: Regex,
    pub slice_re: Regex,
    pub shared_re: Regex,
    pub base_re: Regex,
...
impl FeatureSlicedDesignMatcher {
    ...
    pub fn new(config: FeatureSlicedDesignConfig) -> Self {
        ...
        ///
        /// Without this the four `vocabulary.feature_sliced_design.*` lists were raw regex source spliced into a pattern
        ...
        /// `vocabulary.feature_sliced_design.*` straight here).
        ///
...
impl Default for FeatureSlicedDesignMatcher {
    fn default() -> Self {
        FeatureSlicedDesignMatcher::new(FeatureSlicedDesignConfig::default())
    }
}

#[cfg(test)]
mod tests {
    use super::{FeatureSlicedDesignConfig, FeatureSlicedDesignMatcher};
    ...
    fn cfg_with_entry(entry: &[&str]) -> ScoresConfig {
        ...
        cfg.feature_sliced_design = FeatureSlicedDesignMatcher::new(FeatureSlicedDesignConfig {
            entry: entry.iter().map(|s| s.to_string()).collect(),
            ..cfg.feature_sliced_design.config.clone()
        });
        cfg
    }
