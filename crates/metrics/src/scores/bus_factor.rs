...
pub fn compute_bus_factor(
    ...
    let live: Vec<&FileNode> = nodes
        ...
        .filter(|n| {
            n.loc > 0
                && n.change_count >= min_live_changes
                && n.author_count == 1
                && is_scored(&n.id)
        })
        .collect();
    ...
    let total = nodes
        .iter()
        .filter(|n| n.loc > 0 && n.change_count >= min_live_changes && is_scored(&n.id))
        .count() as u32;
...
mod tests {
    ...
    fn no_eligible_files_change_count_below_10_score_100() {
        let r = compute_bus_factor(&[node("x", 10, 5, 1)], &cfg(), &|_| true);
    ...
    fn empty_input_score_100() {
        let r = compute_bus_factor(&[], &cfg(), &|_| true);
        assert_eq!(r.score, 100.0);
        assert_eq!(r.risky, 0);
    }
    ...
    fn all_eligible_files_single_author_score_0() {
        ...
        let r = compute_bus_factor(
            &[node("a", 10, 12, 1), node("b", 10, 20, 1)],
            &cfg(),
            &|_| true,
        );
    ...
    fn half_of_eligible_files_are_single_author_score_50() {
        ...
        let r = compute_bus_factor(
            &[node("solo", 10, 15, 1), node("team", 10, 15, 3)],
            &cfg(),
            &|_| true,
        );
    ...
    fn loc_0_files_are_excluded_from_both_live_and_total() {
        ...
        let r = compute_bus_factor(
            &[node("ghost", 0, 30, 1), node("team", 50, 30, 4)],
            &cfg(),
            &|_| true,
        );
        // total = 1, live = 0 -> 100
