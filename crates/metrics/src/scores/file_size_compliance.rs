...
//!
//! Non-source files are excluded from BOTH the `violations` list AND the `total`/`compliant`
//! denominator, by the same injected `is_source` closure — see [`super::god_file`]'s module doc for the
//! full rule and why denominator-only exclusion would inflate the ratio.

use super::config::ScoresConfig;
use super::detail_cap::{cap_and_count_dropped, MAX_FILE_ROWS_LISTED};
use super::types::{FileSizeComplianceScore, FileSizeComplianceViolation};
use zzop_core::FileNode;

pub fn compute_file_size_compliance<F>(
    nodes: &[FileNode],
    target: Option<&str>,
    cfg: &ScoresConfig,
    is_source: F,
    is_scored: &dyn Fn(&str) -> bool,
) -> FileSizeComplianceScore
where
    F: Fn(&str) -> bool,
{
    ...
    let live: Vec<&FileNode> = nodes
        ...
        .collect();
    let mut violations: Vec<FileSizeComplianceViolation> = Vec::new();
    let mut compliant = 0u32;
    for n in &live {
        if n.loc <= limit {
            compliant += 1;
        } else {
            violations.push(FileSizeComplianceViolation {
                path: n.path.clone(),
                loc: n.loc,
                limit,
            });
...
mod tests {
    ...
    fn no_live_files_score_100_default_limit_150() {
        let r =
            compute_file_size_compliance(&[], None, &ScoresConfig::default(), |_| true, &|_| true);
    ...
    fn all_files_within_limit_boundary_loc_eq_limit_is_compliant_score_100() {
        ...
        let r =
            compute_file_size_compliance(&nodes, None, &ScoresConfig::default(), |_| true, &|_| {
                true
            });
        assert_eq!(r.score, 100.0);
    ...
    fn half_compliant_score_50() {
        ...
        assert_eq!(
            r.violations,
            vec![FileSizeComplianceViolation {
                path: "big".to_string(),
                loc: 300,
                limit: 150
            }]
    ...
    fn target_be_raises_the_limit_to_200() {
        ...
        let r = compute_file_size_compliance(
            &nodes,
            Some("be"),
