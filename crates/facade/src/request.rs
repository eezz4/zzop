...
pub struct AnalyzeRequest {
    ...
    /// warning naming its `parser` id if invalid (see `envelope::apply_adapter_overlays`). Overlays are
    /// re-applied every run AFTER the native cache, so they need no cache-key participation.
    ///
    /// **Raw JSON on purpose** — typed one overlay at a time by `config::overlays`, whose module doc owns
    /// why (typing the field here made one overlay's shape error the whole request's failure).
    pub adapter_overlays: Vec<serde_json::Value>,
...
pub use envelope::EnvelopeAnalyzeRequest;
