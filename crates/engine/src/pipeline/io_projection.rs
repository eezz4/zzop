...
/// Egress consumes (TS/Python/Rust/Go/C#) run only on a well-formed, in-size-cap file — hence the
...
/// than guess — C# gates only its consumes). Python/Rust/Go ROUTE provides travel as
/// `router_mount_fragments` instead; Python (SQLModel) & Go (GORM) ALSO emit io-direct `db-table`
/// provides too. SQL (`CREATE TABLE`) and Prisma (`model` blocks) are provide-only `db-table` arms with
/// no consume side at all.
///
