...
use super::dep_edges::collect_dep_edges;
...
pub(super) struct FilePassState {
    ...
    /// Ephemeral noncycle-exclusion CANDIDATES (see `zzop_core::noncycle`'s module doc) — never
    /// cached/serialized, alive only for this one `analyze_envelope` call. Every binding that
    /// contributed an edge is recorded with its imported NAME still attached, because the export-side
    /// half of "is this edge erased at compile time?" asks about the TARGET file's declarations, and
    /// this pass streams files one at a time: at the moment `a` is seen importing from `b`, `b`'s
    /// symbols may not have arrived. `ingest` folds it once, after `all_symbols` is complete.
    pub(super) noncycle_candidates: zzop_core::NoncycleCandidates,
...
pub(super) fn run_file_pass(
    ...
    let mut state = FilePassState {
        ...
        io_consumes: Vec::new(),
        dep: DepGraph::new(),
        noncycle_candidates: zzop_core::NoncycleCandidates::new(),
    ...
    for file in files {
        ...
        collect_dep_edges(file, all_paths, &mut state);
