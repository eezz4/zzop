...
fn git_enabled_populates_scores_health_recommendations_critical_and_seams() {
    ...
    // `pain` is `Option<f64>` since 2026-08-08: `None` means not one metric had a population to score
    // over. This fixture has a real dep graph and git history, so it must be a measured number.
    let pain = health
        .pain
        .expect("a tree with imports and git history measured at least one metric");
    assert!(pain.is_finite());
    assert!(pain >= 0.0);
    assert!(
        health.measured_weight > 0.0 && health.measured_weight <= health.total_weight,
        "pain must ship the weight it was renormalized over"
    );
...
/// `crates/core/src/dsl.rs`'s `RulePackDef`/`RuleDef`/`Matcher::LineScan` for the field shapes this
