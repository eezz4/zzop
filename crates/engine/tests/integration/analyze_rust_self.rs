...
fn workspace_tree() -> TempDir {
    ...
    // A `#[path = "..."]` module declaration: the module is named `aliased` but its file is not
    // `aliased.rs`, and the file is not under `alias_host/` either. Both facts are needed to reproduce
    // the defect this fixture pins — see the self-edge test below.
    dir.write(
        "crates/app/src/alias_host.rs",
        concat!(
            "#[path = \"renamed_target.rs\"]\n",
            "mod aliased;\n",
            "\n",
            "pub fn host() -> i32 {\n",
            "    aliased::value()\n",
    ...
    dir.write(
        "crates/app/src/renamed_target.rs",
        concat!("pub fn value() -> i32 {\n", "    7\n", "}\n",),
    );
    dir
}
...
/// `#[path = "..."]` names the module's file OUTRIGHT — the `foo.rs`/`foo/mod.rs` convention does not
/// apply — so the edge must land on the named file.
#[test]
fn a_path_attribute_module_edge_lands_on_the_named_file() {
    let dir = workspace_tree();
    let out = analyze_tree(dir.path(), &config());
    let targets = out.ir.ir.dep.get("crates/app/src/alias_host.rs");
    assert!(
        targets.is_some_and(|t| t.iter().any(|x| x == "crates/app/src/renamed_target.rs")),
        "expected alias_host.rs -> renamed_target.rs #[path] edge, got: {targets:?}"
    );
}

/// The failure this actually was, and the reason a parser-level unit test cannot see it: the old
/// behaviour was NOT silence. `#[path]` was ignored, so `mod aliased;` resolved by convention, missed
/// `alias_host/aliased.rs`, and fell through to the item-vs-module parent candidate — which is
/// `alias_host.rs` ITSELF. Every one of this repo's 13 such declarations therefore produced a
/// **self-edge**, inflating that file's own degree while the real target sat at fan-in 0 looking dead.
/// Same class as the 2026-07-29 cross-crate finding: the edge existed and pointed at the wrong place,
/// which is strictly worse than absent because a missing edge is visible as an island and a self-edge
/// is not.
#[test]
fn a_path_attribute_module_never_produces_a_self_edge() {
    let dir = workspace_tree();
    let out = analyze_tree(dir.path(), &config());
    let targets = out.ir.ir.dep.get("crates/app/src/alias_host.rs");
    assert!(
        targets.is_some_and(|t| !t.iter().any(|x| x == "crates/app/src/alias_host.rs")),
        "alias_host.rs must not depend on itself, got: {targets:?}"
    );
}
