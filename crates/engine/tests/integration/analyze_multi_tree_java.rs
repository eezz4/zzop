...
fn java_be_tree() -> TempDir {
    // Prefix is unique ACROSS test files on purpose (analyze_multi_tree.rs also builds a Java BE
    // tree): TempDir's uniquifying counter is a per-file static, so once these files share one
    // harness binary, an identical prefix at the same nanos tick names the same directory —
    // `create_dir_all` raises no error, and two tests silently share and double-Drop one dir.
    let dir = TempDir::new("zzop-engine-multi-tree-java");
    dir.write(
