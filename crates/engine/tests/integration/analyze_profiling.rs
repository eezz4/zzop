...
/// The real `rules/dsl/security/security.json`, filtered to three Java security-concern LINE-SCAN
/// rules (`sql-string-concat`/`weak-cipher`/`cmd-injection`) that trace back to the dissolved
/// language-named `java-security` pack (v0.15; `weak-crypto` was the third member until 2026-08-09,
/// when its hash half became a six-language call-scan rule and its cipher arms split out as
/// `weak-cipher`) — all three share a `.java`-ish `file_pattern`, so every one of them gets
/// evaluated, and therefore timed, against any `.java` file the pack applies to (see
...
/// `security/sql-string-concat` (exercises a DSL finding; `weak-cipher`/`cmd-injection` still run against this
/// same file — they just don't fire, since the file has no cipher-construction/exec pattern). No git history is
...
fn sql_taint_dsl_rule_timing_reflects_the_finding_it_produced() {
    ...
    // weak-cipher/cmd-injection still ran (same pack, same file) but matched nothing in this fixture.
    for id in ["security/weak-cipher", "security/cmd-injection"] {
        let t = timings
            .iter()
            .find(|t| t.rule_id == id)
