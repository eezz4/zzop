//! Line-scan matcher tests: the security `sql-string-concat`/`weak-cipher` rules end-to-end, plus the
...
// --- weak-cipher (the line-scan half of the old weak-crypto; the hash half is call-scan since
// --- 2026-08-09 and is exercised through the real engine in `rules/dsl/security`'s pack tests) ---

#[test]
fn digest_lines_left_the_line_scan_set_with_the_call_scan_migration() {
    // Boundary pin, both directions stated: `MessageDigest.getInstance("MD5")` now reports through
    // the projected `hash-call` channel (which this module's bare `SourceFile` deliberately lacks),
    // and the commons-codec `DigestUtils.md5*` helper family reports NOWHERE — its algorithm lives in
    // a method name, not a construction argument the channel witnesses (zero uses measured in corpus
    // and cases at migration time; the rule's own message discloses it). If either line ever matches
    // a line-scan rule again, someone re-added a digest arm without reconciling the migration.
    assert!(scan(
...
#[test]
fn ignores_weak_cipher_in_comments() {
    assert!(scan(
        r#"// legacy used Cipher.getInstance("DES/CBC/PKCS5Padding") here"#,
        "C.java"
    )
    .is_empty());
}
