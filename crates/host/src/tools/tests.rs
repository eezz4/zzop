...
//! `source: "inline"` for every bundled pack). These drive the shared CLI-facing wrappers
//! (`super::analyze`/`super::cross_repo`/`super::check_endpoint`/`super::validate_*`) directly — the
//! same functions the `zzop` CLI's subcommands call — so they pin the thin facade, not the shaping
//! logic itself (that logic's own unit tests now live beside it in `zzop-summary`, e.g.
//! `zzop_summary`'s `config_warnings` module). The MCP-surface half of this coverage (`tools/list`
//! schema pins and `tools/call` dispatch, through the real wire shape) lives in the `zzop-mcp` package
//! instead (`packages/mcp/src/tools/tests.rs`), since `tools/list`/`tools/call` are MCP-only and no
//! longer live in this crate.
...
fn cli_validate_wrappers_return_the_same_verdict_as_the_tool() {
    ...
    // same `zzop_summary` checks the MCP `validate_rule_pack`/`validate_envelope` tools do (packages/mcp)
    // — so a terminal check and a tool call agree by construction.
