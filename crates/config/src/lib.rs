//! zzop-config — the config front-end shared by every native host (the zzop-mcp binary today; the
//! binary's own CLI surface as it grows). It turns a `zzop.config.jsonc` (or its absence) into a
//! `zzop-facade` request JSON, so every host drives one engine code path (`zzop-facade`'s
//! `build_engine_config`) from a single config implementation.
//!
