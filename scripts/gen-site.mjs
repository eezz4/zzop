...
function bodyFor(mode) {
  const nav = PAGES.map(
    (p, i) =>
      `<a href="#${p.id}" class="nav__link${i === 0 ? " is-here" : ""}" data-page="${p.id}">` +
  ...
  // The language switch is a LINK to the page in the other language. It carries the tab you are
  // looking at across as a hash. The visible text (EN / KO) is language-neutral and stays here.
  const langBtn =
...
(function () {
  ...
  // deep link straight into a page
