...
// A pair counts as separated once it is within R_EPS of touching, NOT at exact float equality, and that
// distinction is load-bearing (2026-08-14). A push places the pair at exactly `need`, but the arithmetic
// that gets it there rounds, so `d` lands a few ULPs short and the pass re-flags the pair forever. It
// stayed invisible while every pair happened to relax with slack to spare; adding eleven files to the
// tree moved two small domains to exact contact and the loop ran 20000 passes without settling, with a
// measured worst overlap of 2.8e-14 px. The throw below would then have called that a smear. R_EPS is
// eight orders of magnitude below one device pixel and eight above the residue, so it cannot hide an
// overlap a reader could see.
const R_EPS = 1e-6;
let settled = false;
...
if (!settled) {
  // Report the worst residual so the next reader can tell a real smear from float residue without
  // instrumenting this loop again — that measurement is what turned a false alarm into a one-line fix.
  let worst = 0;
  for (let i = 0; i < bubbles.length; i++) {
    for (let j = i + 1; j < bubbles.length; j++) {
      const A = bubbles[i], B = bubbles[j];
      worst = Math.max(worst, A.r + B.r + R_PAD - Math.hypot(B.bx - A.bx, B.by - A.by));
    }
  }
  throw new Error(
    `domain bubbles never stopped colliding — the summary would draw a smear over the domain names ` +
      `(worst overlap ${worst} px after ${bubbles.length} bubbles relaxed; anything near 1e-14 is float ` +
      `residue, not a layout that failed)`
  );
}
...
//
// THAT site/architecture.html ANCHOR IS GONE (2026-08-14), and the reason is worth writing down because
// it is the OPPOSITE of the drift above rather than a retreat from it. The site was rebuilt as one page
// of tabs generated by scripts/gen-site.mjs from site-src/content/*.mjs; architecture.html is now a
// redirect stub with no content of its own, so the sentence this script owned no longer exists — the
...
// read on first use. Every anchor happens to sit on graph.html today, but the map and the loop below
// stay keyed by FILE — that shape is what caught the architecture.html copy in the first place, and
// collapsing it back to a single page would re-open the exact hole documented above.
// Nothing is written until every anchor has matched, so a hard error leaves the
...
console.log(
  `site/graph.html: ${rows.length} nodes, ${links.length} links, ${colKeys.length} columns` +
    ...
    ` · prose counts rewritten in ${buffers.size} file(s) (${commas(nodeCount)} files, ${commas(linkCount)} imports)`
);
