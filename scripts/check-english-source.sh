...
FOREIGN_LETTER_EXEMPTIONS=(
  ...
  "site/ko/|the GENERATED Korean editions, and generated is the whole reason they are excusable: each is byte-for-byte the output of the site generator (scripts/gen-site.mjs — spelled WITHOUT its interpreter on purpose, so this sentence cannot be read as an invocation site by check-guards-wired.sh) over the pairs above, which scripts/check-site-generated.sh re-derives and compares on every commit — so no Hangul can enter these files except through a {ko, en} pair that also produced an English sentence. They are committed because GitHub Pages serves site/ as files. Their English twins at the root of site/ are deliberately NOT exempt and must stay that way; that asymmetry is the guard. THIS ONE IS A DIRECTORY, and it stays honest only because that same drift check ALSO refuses any file under site/ko/ the generator did not write. It was per-PAGE entries until 2026-08-18 and grew by one the first day a second page was translated — a hand whitelist that grows per page is the shape this repo keeps being bitten by, and the fix was to make the directory claim checkable rather than to keep adding lines. VOID THIS ENTRY if that refusal is removed from check-site-generated.sh, if any page here is ever hand-edited or stops being generated, or if site/ stops being one-language-per-file."
)
